...
export const vocabularyService = {
  ...
  async fetchVocabularyData(): Promise<VocabItem[]> {
    try {
      ...
      if (!token) {
        ...
        console.log(
          'No authentication token found - loading anonymous vocabulary'
        )
  ...
  async toggleMemorizedStatus(
    word: VocabItem,
    willBeMemorized: boolean
  ): Promise<{ success: boolean; error?: string }> {
    if (willBeMemorized) {
  ...
  async getUserSpreadsheetId(): Promise<{
    success: boolean
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/get-user-spreadsheet`,
        getApiOptions()
      )
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to fetch spreadsheet ID',
      }
    }
  },

  // Create a new Google Sheet for the user
  async createUserSpreadsheet(): Promise<{
    success: boolean
    spreadsheetId?: string
    error?: string
    isNew?: boolean
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/create-user-spreadsheet`,
        getApiOptions('POST', {}, true)
      )
