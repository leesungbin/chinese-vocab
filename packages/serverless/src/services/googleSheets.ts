...
export class GoogleSheetsService {
  ...
  async addVocabWord(word: VocabWord, range: string = 'Sheet1!A:H'): Promise<void> {
    try {
      const values = [[
        word.id.toString(),
        word.day.toString(),
  ...
  async updateVocabWord(word: VocabWord, rowIndex: number, range: string = 'Sheet1'): Promise<void> {
    try {
  ...
  async markAsMemorized(chinese: string, range: string = 'Sheet1!A:H'): Promise<void> {
    try {
  ...
  async incrementTotalAndUpdateLastReviewed(chinese: string, range: string = 'Sheet1!A:H'): Promise<void> {
    try {
      // First, get all words to find the correct row
      const words = await this.getVocabWords(range);
      const rowIndex = words.findIndex(word => word.chinese === chinese);
  ...
  async deleteVocabWord(chinese: string, range: string = 'Sheet1!A:H'): Promise<void> {
    try {
      // First, get all words to find the correct row
      const words = await this.getVocabWords(range);
      const rowIndex = words.findIndex(word => word.chinese === chinese);
