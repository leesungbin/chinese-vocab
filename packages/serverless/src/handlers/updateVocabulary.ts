  ...
  try {
    ...
    switch (request.action) {
      ...
      case 'increment_total':
        await dynamoService.incrementTotalAndUpdateLastReviewed(
          user.userId,
          request.word.id
        )
      ...
      case 'delete':
        throw new Error(
          `Action '${request.action}' is not supported. Static vocabulary data can only be managed via migration.`
        )
